...
def prepare(root: str, output: str) -> None:
    ...
    train_df['timestamp'] = pd.to_datetime(train_df['timestamp'], format=ts_format)
    test_df['timestamp'] = pd.to_datetime(test_df['timestamp'], format=ts_format)
    weather_train_df['timestamp'] = pd.to_datetime(weather_train_df['timestamp'], format=ts_format)
    weather_test_df['timestamp'] = pd.to_datetime(weather_test_df['timestamp'], format=ts_format)
    ...
    for bid in train_df['building_id'].unique():
        ...
        for meter in df['meter'].unique():
            ...
            train_df.loc[
                (train_df['building_id'] == bid) & (train_df['meter'] == meter), 'black_count'
            ] = b.groupby((~b.astype(bool)).cumsum()).cumsum()
