...
class GaussianTargetEncoder:
    def __init__(
        self, group_cols: List[str], target_col: str = "target", prior_cols: Optional[List[str]] = None
    ):
    ...
    def transform(
        self, df: pd.DataFrame, prior_precision: float = 1e-6, stat_type: str = "mean"
    ) -> np.ndarray:
        ...
        if stat_type == "mean":
            numer = (
                prior_precision * self.stats["mu_prior"]
    ...
    def fit_transform(self, df: pd.DataFrame, *args: Any, **kwargs: Any) -> np.ndarray:
        self.fit(df)
